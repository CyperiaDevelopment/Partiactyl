...
async function partyPing() {
    let params = JSON.stringify({
                content: "<@&858606628683907084>"
            })
            fetch(`${settings.api.client.party.webhook}`, {
                method: "POST",
                headers: {
...
async function disableParty() {
    let params = JSON.stringify({
                embeds: [
                    {
                        title: "Party Mode",
                        description: `${settings.api.client.party.disabled_message}`,
                        color: hexToDecimal("#ffff00")
                    }
                ]
            })
...
module.exports.load = async function(app, db) {
    setInterval(async () => {
        let arcstuff= await fetch("https://client.hexitnodes.com/getlivearc" );
    	let arcjson = await arcstuff.json();
        if(arcjson.widgets >= 120) {
            if(partyon !== true) {
                enableParty()
                partyPing()
                partyon = true;
  ...
  app.get("/api/leaderboard", async (req, res) => {
      ...
      for(let i = 0; i < userdb.length; i++) {
          let e = userdb[i];
          let t = await db.get("coins-"+e);
          let username = await db.get("username-"+e);
          username = username ? username : "Unknown User";
          await leaders.push({id:`${e}`,coins:t, username:`${username}`});
      };
      let sorted = leaders.sort((a, b) => (b.coins > a.coins) ? 1 : -1)
  ...
  app.ws("/" + settings.api.arcio["afk page"].path, async (ws, req) => {
    ...
    let coinloop = setInterval(
      async function() {
          ...
          if(!userdb.includes(`${req.session.userinfo.id}`)) {
              userdb.push(`${req.session.userinfo.id}`);
              await db.set("userlist", userdb);
          }
          await db.set("username-"+req.session.userinfo.id, req.session.userinfo.username);
          let arcstuff= await fetch("https://client.hexitnodes.com/getlivearc" );
    	  let arcjson = await arcstuff.json();
          if(arcjson.widgets >= 120) {
        let usercoins = await db.get("coins-" + req.session.userinfo.id);
        usercoins = usercoins ? usercoins : 0;
        usercoins = usercoins + (newsettings.api.arcio["afk page"].coins * 2);
